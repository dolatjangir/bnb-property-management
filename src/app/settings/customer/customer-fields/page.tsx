...
import { getPropertyFieldLabel, updatePropertyFieldLabel } from "@/store/settings/property/propertyfields/propertyfields";
...
export default function PropertyFields() {
    ...
    const { updateLabel } = usePropertyFieldLabel();

    // 🔹 Load saved labels
    useEffect(() => {
        const init = async () => {
            const savedLabels = await getPropertyFieldLabel(); // { key: label }

            const merged = PROPERTY_FIELD_KEYS.map((key) => ({
                fieldKey: key,
    ...
    const handleSave = async () => {
        ...
        const res = await updatePropertyFieldLabel(payload);

        if (res) {
            toast.success("Property fields updated successfully");
