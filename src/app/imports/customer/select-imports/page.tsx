...
import { addProperty, importProperty } from "@/store/property";
import { propertyAllDataInterface, propertyImportDataInterface } from "@/store/property.interface";
...
export default function SelectImports() {
    ...
    const { excelHeaders, file } = usePropertyImport();
    const [propertyFieldMasters, setPropertyFieldMasters] = useState<string[]>([]);
    ...
    useEffect(() => {
        if (excelHeaders.length === 0) {
            ...
            router.push("/imports/property");
        }
    }, [excelHeaders]);

    useEffect(() => {
        const loadPropertyFields = async () => {
            const res = await getPropertyFields(); 
             const activeFields = res.filter((e: any) => e.Status === "Active");
            setPropertyFieldMasters(activeFields.map((f: any) => f.Name));
        };
        loadPropertyFields();
    }, []);
    ...
    const handleSubmit = async () => {
        ...
        const result = await importProperty(formData);
    ...
    const propertyFields =[  ...propertyFieldMasters,...systemFields];
                    ...
                    <form onSubmit={(e) => e.preventDefault()} className="w-full">
                        <div className="mb-8 text-left border-b pb-4 border-gray-200">
                            <h1 className="text-3xl font-extrabold text-[var(--color-secondary-darker)] leading-tight tracking-tight">
                                Property <span className="text-[var(--color-primary)]">Selected Imports</span>
                            </h1>
                        </div>
