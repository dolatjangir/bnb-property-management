...
import { propertyExcelHeaders, importProperty } from "@/store/property";
...
import { usePropertyImport } from "@/context/PropertyImportContext";
export default function PropertyImport() {
  ...
  const { setExcelHeaders, setFile } = usePropertyImport();
  ...
  const objectFields = [
    ...
    { key: "PropertySubtype", staticData: [] } // dependent
  ...
  useEffect(() => {
    if (importData.Campaign.id) {
      fetchPropertyType(importData.Campaign.id);
    } else {
      setFieldOptions((prev) => ({ ...prev, PropertyType: [] }));
    }

    if (importData.Campaign.id && importData.PropertyType.id) {
      fetchPropertySubType(importData.Campaign.id, importData.PropertyType.id);
    } else {
      setFieldOptions((prev) => ({ ...prev, PropertySubtype: [] }));
    }
  }, [importData.Campaign.id, importData.PropertyType.id]);
  ...
  const fetchPropertySubType = async (campaignId: string, propertytypeId: string) => {
    try {
      const res = await getSubtypeByCampaignAndType(campaignId, propertytypeId);
      setFieldOptions((prev) => ({ ...prev, PropertySubtype: res || [] }));
    } catch (error) {
      console.error("Error fetching types:", error);
      setFieldOptions((prev) => ({ ...prev, PropertySubtype: [] }));
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      formData.append("PropertyType", importData.PropertyType?.name);
      formData.append("PropertySubType", importData.PropertySubType?.name); */
      if (importData.file) formData.append("file", importData.file);

      console.log(importData)
      // property import api call
      /*  const result = await importProperty(formData); // mock success */
      /* property header select api call  */
      const result = await propertyExcelHeaders(formData);

      if (result?.headers) {
        console.log(" result ", result)
        // toast.success(`${result.importedCount} propertys imported successfully. ${result.skippedCount} duplicates skipped.`);
        setExcelHeaders(result.headers);
        router.push("/imports/property/select-imports/");
      } else {
        toast.error("Failed to import data");
      }
    } catch (error) {
      console.error("Import Error:", error);
      toast.error("Error importing propertys");
    }
  };
  ...
  const handleDownload = () => {
    ...
    link.href = "/files/property-import-file.xlsx";
