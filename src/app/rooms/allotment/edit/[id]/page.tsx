...
import { getRoomAllotmentById, updateRoomAllotment } from "@/store/room/roomallotment/roomallotment";
...
export default function RoomEdit() {
    ...
    const validateForm = () => {
        ...
        if (!roomAllotmentData.Room?.name) newErrors.Room = " Room is required";
        if (!roomAllotmentData.Guest.trim()) newErrors.Floor = "Guest is required";
        return newErrors;
    };

    // Fetch existing data
    useEffect(() => {
        const fetchRoomAllotmentData = async () => {
            const res = await getRoomAllotmentById(id as string);
            console.log(res)
            if (res) {
                setRoomAllotmentData({
                    Property: res.Property,
                    Guest: res.Guest,
                    Room: { id: res.Room.id, name: res.Room }, 
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const result = await updateRoomAllotment(id as string, payload);
            if (result) {
                toast.success(" Room updated successfully!");
                router.push("/rooms/allotment");
            }
        } catch (error) {
