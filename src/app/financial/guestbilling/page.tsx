...
import {
    GuestBillingGetDataInterface,
    GuestBillingDialogDataInterface,
} from "@/store/financial/guestbilling/guestbilling.interface";

import {
    deleteGuestBilling,
    getFilteredGuestBilling,
    getGuestBilling,
} from "@/store/financial/guestbilling/guestbilling";
...
export default function GuestBilling() {
    ...
    const [guestBillingData, setGuestBillingData] = useState<GuestBillingGetDataInterface[]>([]);
    ...
    const [deleteDialogData, setDeleteDialogData] = useState<GuestBillingDialogDataInterface | null>(null);
    ...
    const getExpenseList = async () => {
        const data = await getGuestBilling();
        if (data) {
            const formatted = data.map((t: any) => ({
                _id: t._id,
                Guest: t.Guest.GuestName.charAt(0).toUpperCase() + t.Guest.GuestName.slice(1),
                Amount: t.Amount,
    ...
    const handleSelectChange = async (
        ...
        const data = await getFilteredGuestBilling(queryParams.toString());
    ...
    const handleDelete = async () => {
        if (!deleteId) return;
        const res = await deleteGuestBilling(deleteId);
        if (res) {
    ...
    // Pagination logic
    const totalPages = Math.ceil(guestBillingData.length / rowsPerTablePage) || 1;
    const startIndex = (currentPage - 1) * rowsPerTablePage;
    const currentRows = guestBillingData.slice(startIndex, startIndex + rowsPerTablePage);
    ...
    return (
        <ProtectedRoute>
            <div className="min-h-[calc(100vh-56px)] overflow-auto max-md:py-10">
                ...
                {/* DELETE POPUP */}
                <DeleteDialog<GuestBillingDialogDataInterface>
                ...
                <div className="p-4 w-full bg-white rounded-md">
                    ...
                    <div className="flex justify-between items-center">
                        <PageHeader title="Dashboard" subtitles={["Finance", "Expenditure"]} />


                        <AddButton
                            url="/financial/guestbilling/add"
                            text="Add"
                            ...
                            <table className="table-auto w-full border-collapse text-sm border border-gray-200">
                                <thead className="bg-[var(--color-primary)] text-white">
                                    <tr>
                                        ...
                                        <th className="px-4 py-3 border border-[var(--color-secondary-dark)] text-left">Guest</th>
                                        <th className="px-4 py-3 border border-[var(--color-secondary-dark)] text-left">Amount</th>
                                        <th className="px-4 py-3 border border-[var(--color-secondary-dark)] text-left">Recieved Amount</th>
                                        ...
                                        <th className="px-4 py-3 border border-[var(--color-secondary-dark)] text-left">From Date</th>
                                        <th className="px-4 py-3 border border-[var(--color-secondary-dark)] text-left">To Date</th>
                                    ...
                                    {currentRows.length > 0 ? (
                                        currentRows.map((item, index) => (
                                            ...
                                            >
                                                ...
                                                <td className="px-4 py-3 border border-gray-200">{item.Guest}</td>
                                                <td className="px-4 py-3 border border-gray-200">{item.Amount}</td>
                                                <td className="px-4 py-3 border border-gray-200">{item.RecievedAmount}</td>
                                                <td className="px-4 py-3 border border-gray-200">{item.DueAmount}</td>
                                                <td className="px-4 py-3 border border-gray-200">{item.PaymentMethod}</td>
                                                <td className="px-4 py-3 border border-gray-200">{item.FromDate}</td>
                                                <td className="px-4 py-3 border border-gray-200">{item.ToDate}</td>
                                                <td className="px-4 py-2 flex gap-2 items-center">
                                                    <Button
                                                        ...
                                                        onClick={() =>
                                                            router.push(`/financial/guestbilling/edit/${item._id}`)
                                                        }
                                                    >
                                                        <MdEdit />
                                                    </Button>
