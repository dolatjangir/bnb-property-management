...
export default function RoomTable<T extends Record<string, any>>({
    leads,
    labelLeads,
    onEdit,
    onFavourite,
            ...
            <div className="px-0 pb-6 space-y-3">
                ...
                {paginatedLeads.length === 0 && (
                    <div className="flex flex-col items-center justify-center py-16 gap-3">
                        ...
                        <p className="text-sm text-gray-400 font-medium">No rooms available</p>
                    </div>
                )}
                ...
                {paginatedLeads.map((lead, index) => (
                    <div
                        key={index}
                        className="room-card relative bg-white rounded-2xl overflow-hidden transition-all duration-300"
                        style={{
                    ...
                    >
                        {/* Left accent bar */}
                        <div
                            className="absolute left-0 top-0 bottom-0 w-1 rounded-l-2xl bg-[var(--color-primary)]"
                        />

                        <div className="pl-5 pr-4 pt-4 pb-0">
                            {/* Top row: first label as title + edit button */}
                            <div className="flex items-start justify-between gap-3 mb-3">
                                <div className="flex-1 min-w-0">
                                    {/* First field as card "title" */}
                                    {labelLeads[0] && (
                                ...
                                <button
                                    ...
                                    className="edit-btn shrink-0 w-9 h-9 rounded-xl bg-[var(--color-primary)]/10 flex items-center justify-center text-[var(--color-primary)] transition-transform duration-200"
                                >
                                    <MdEdit size={17} />
                                </button>
                            </div>
                            ...
                            {/* Remaining fields as pill-style tags */}
