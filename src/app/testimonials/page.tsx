...
import { testimonialsDialogDataInterface, testimonialsGetDataInterface } from "@/store/testimonials/testimonials.interface";
import { deleteTestimonials, getTestimonials } from "@/store/testimonials/testimonials";


export default function TestimonialsPage() {
    const [testimonials, setTestimonials] = useState<testimonialsGetDataInterface[]>([]);
    const [keyword, setKeyword] = useState("");
    const [limit, setLimit] = useState("10");
    const [isDeleteDialogOpen, setIsDeleteDialogOpen] = useState(false);
    const [deleteDialogData, setDeleteDialogData] = useState<testimonialsDialogDataInterface | null>(null);
    ...
    const fetchTestimonials = async () => {
        ...
        if (data) {
            const formattedData = data.map((r: testimonialsGetDataInterface) => ({
                ...r,
                Name: r.Title.charAt(0).toUpperCase() + r.Title.slice(1)
            }));
    ...
    }, [testimonials, keyword, limit]);

    // Delete testimonials
    const handleDelete = async (data: testimonialsDialogDataInterface | null) => {
        if (!data) return;
        const res = await deleteTestimonials(data.id);
        if (res) {
    ...
    const totalTablePages = Math.ceil(filteredTestimonials.length / rowsPerTablePage);
    const indexOfLastRow = currentTablePage * rowsPerTablePage;
    const indexOfFirstRow = indexOfLastRow - rowsPerTablePage;
    const currentRows = filteredTestimonials.slice(indexOfFirstRow, indexOfLastRow);
    ...
    return (
        <MasterProtectedRoute>
            ...
            <div className="min-h-[calc(100vh-56px)] overflow-auto max-md:py-10">
                ...
                {/* Delete Dialog */}
                <DeleteDialog<testimonialsDialogDataInterface>
                ...
                <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-200 relative">
                    <PageHeader title="Dashboard" subtitles={["Testimonials"]} />
                    {/* Add Button */}
                            ...
                            <thead className="bg-[var(--color-primary)] text-white">
                                <tr className="flex justify-between items-center w-full">
                                    <th className="flex items-center gap-10 px-8 py-3 border border-[var(--color-secondary-dark)] text-left w-1/2">
                                        ...
                                        <p className="w-[120px] text-left">Description</p>
                                    </th>
                                    ...
                                    currentRows.map((r, i) => (
                                        ...
                                        >
                                            <td className="flex items-center gap-10 px-8 py-3 w-1/2">
                                                ...
                                                <p className="w-[200px] font-semibold">{r.Description}</p>
                                            </td>
