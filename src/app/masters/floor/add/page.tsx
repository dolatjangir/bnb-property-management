...
import { addFloor } from "@/store/masters/floor/floor";
...
export default function FloorAdd() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const result = await addFloor(floorData);
      if (result) {
      ...
      console.error("Floor Add Error:", error);
    }
  };
            ...
            <form onSubmit={(e) => e.preventDefault()} className="w-full">
              <div className="mb-8 text-left border-b pb-4 border-gray-200">
                <h1 className="text-3xl font-extrabold text-[var(--color-secondary-darker)] leading-tight tracking-tight">
                  Add <span className="text-[var(--color-primary)]">Floor</span>
                </h1>
              </div>
