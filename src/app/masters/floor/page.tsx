...
import { floorDialogDataInterface, floorGetDataInterface } from "@/store/masters/floor/floor.interface";
import { deleteFloor, getFloor } from "@/store/masters/floor/floor";


export default function FloorPage() {
    const [floor, setFloor] = useState<floorGetDataInterface[]>([]);
    const [keyword, setKeyword] = useState("");
    const [limit, setLimit] = useState("10");
    const [isDeleteDialogOpen, setIsDeleteDialogOpen] = useState(false);
    const [deleteDialogData, setDeleteDialogData] = useState<floorDialogDataInterface | null>(null);
    ...
    const fetchFloor = async () => {
        ...
        if (data) {
            const formattedData = data.map((r: floorGetDataInterface) => ({
                ...r,
    ...
    // Delete floor
    const handleDelete = async (data: floorDialogDataInterface | null) => {
        if (!data) return;
        const res = await deleteFloor(data.id);
        if (res) {
    ...
    const handleEdit = (id?: string) => {
        router.push(`/masters/floor/edit/${id}`);
    };
    ...
    const totalTablePages = Math.ceil(filteredFloor.length / rowsPerTablePage);
    const indexOfLastRow = currentTablePage * rowsPerTablePage;
    const indexOfFirstRow = indexOfLastRow - rowsPerTablePage;
    const currentRows = filteredFloor.slice(indexOfFirstRow, indexOfLastRow);
    ...
    return (
        <MasterProtectedRoute>
            ...
            <div className="min-h-[calc(100vh-56px)] overflow-auto max-md:py-10">
                ...
                {/* Delete Dialog */}
                <DeleteDialog<floorDialogDataInterface>
                ...
                <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-200 relative">
                    <PageHeader title="Dashboard" subtitles={["Floor"]} />
                    {/* Add Button */}

                    <AddButton
                        url="/masters/floor/add"
                        text="Add"
