...
import { roomtypeDialogDataInterface, roomtypeGetDataInterface } from "@/store/masters/roomtype/roomtype.interface";
import { deleteRoomType, getRoomType } from "@/store/masters/roomtype/roomtype";


export default function RoomTypePage() {
    const [roomtype, setRoomType] = useState<roomtypeGetDataInterface[]>([]);
    const [keyword, setKeyword] = useState("");
    const [limit, setLimit] = useState("10");
    const [isDeleteDialogOpen, setIsDeleteDialogOpen] = useState(false);
    const [deleteDialogData, setDeleteDialogData] = useState<roomtypeDialogDataInterface | null>(null);
    ...
    const fetchRoomType = async () => {
        ...
        if (data) {
            const formattedData = data.map((r: roomtypeGetDataInterface) => ({
                ...r,
    ...
    // Filtered roomtype
    const filteredRoomType = useMemo(() => {
        return roomtype
            .filter((r) => keyword === "" || r.Name.toLowerCase().includes(keyword.toLowerCase()))
    }, [roomtype, keyword, limit]);

    // Delete roomtype
    const handleDelete = async (data: roomtypeDialogDataInterface | null) => {
        if (!data) return;
        const res = await deleteRoomType(data.id);
        if (res) {
        ...
        toast.error("Failed to delete roomtype.");
    };

    // Edit roomtype
    const handleEdit = (id?: string) => {
        router.push(`/masters/roomtype/edit/${id}`);
    };
    ...
    const totalTablePages = Math.ceil(filteredRoomType.length / rowsPerTablePage);
    const indexOfLastRow = currentTablePage * rowsPerTablePage;
    const indexOfFirstRow = indexOfLastRow - rowsPerTablePage;
    const currentRows = filteredRoomType.slice(indexOfFirstRow, indexOfLastRow);
    ...
    return (
        <MasterProtectedRoute>
            ...
            <div className="min-h-[calc(100vh-56px)] overflow-auto max-md:py-10">
                ...
                {/* Delete Dialog */}
                <DeleteDialog<roomtypeDialogDataInterface>
                    isOpen={isDeleteDialogOpen}
                    title="Are you sure you want to delete this roomtype?"
                ...
                <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-200 relative">
                    <PageHeader title="Dashboard" subtitles={["RoomType"]} />
                    {/* Add Button */}

                    <AddButton
                        url="/masters/roomtype/add"
                        text="Add"
                    ...
                    <form className="w-full flex flex-wrap gap-6 items-end mb-6 mt-16">
                        ...
                        <div className="flex flex-col flex-1 w-60">
                            ...
                            <input
                                id="keyword"
                                type="text"
                                placeholder="Search by roomtype name..."
                                value={keyword}
