...
import { propertyFieldsDialogDataInterface, propertyFieldsGetDataInterface } from "@/store/masters/propertyfields/propertyfields.interface";
import { deletePropertyFields, getPropertyFields } from "@/store/masters/propertyfields/propertyfields";



export default function PropertyFieldsPage() {
    const [propertyFields, setPropertyFields] = useState<propertyFieldsGetDataInterface[]>([]);
    const [keyword, setKeyword] = useState("");
    const [limit, setLimit] = useState("10");
    const [isDeleteDialogOpen, setIsDeleteDialogOpen] = useState(false);
    const [deleteDialogData, setDeleteDialogData] = useState<propertyFieldsDialogDataInterface | null>(null);
    ...
    const fetchPropertyFields = async () => {
        ...
        if (data) {
            const formattedData = data.map((r: propertyFieldsGetDataInterface) => ({
                ...r,
                Name: r.Name.charAt(0).toUpperCase() + r.Name.slice(1)
            }));
            setPropertyFields(formattedData);
        }
    };
    ...
    // Filtered propertyFields
    const filteredPropertyFields = useMemo(() => {
        return propertyFields
            .filter((r) => keyword === "" || r.Name.toLowerCase().includes(keyword.toLowerCase()))
    }, [propertyFields, keyword, limit]);

    // Delete propertyFields
    const handleDelete = async (data: propertyFieldsDialogDataInterface | null) => {
        if (!data) return;
        const res = await deletePropertyFields(data.id);
        if (res) {
            toast.success("PropertyFields deleted successfully!");
            setIsDeleteDialogOpen(false);
            setDeleteDialogData(null);
            fetchPropertyFields();
            return;
        }
        toast.error("Failed to delete propertyFields.");
    };

    // Edit propertyFields
    const handleEdit = (id?: string) => {
        router.push(`/masters/propertyfields/edit/${id}`);
    };
    ...
    const totalTablePages = Math.ceil(filteredPropertyFields.length / rowsPerTablePage);
    const indexOfLastRow = currentTablePage * rowsPerTablePage;
    const indexOfFirstRow = indexOfLastRow - rowsPerTablePage;
    const currentRows = filteredPropertyFields.slice(indexOfFirstRow, indexOfLastRow);
    ...
    return (
        <MasterProtectedRoute>
            ...
            <div className="min-h-[calc(100vh-56px)] overflow-auto max-md:py-10">
                ...
                {/* Delete Dialog */}
                <DeleteDialog<propertyFieldsDialogDataInterface>
                    isOpen={isDeleteDialogOpen}
                    title="Are you sure you want to delete this propertyFields?"
                ...
                <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-200 relative">
                    <PageHeader title="Dashboard" subtitles={["PropertyFields"]} />

                    {/* Add Button */}
                    <AddButton
                        url="/masters/propertyfields/add"
                        text="Add"
                    ...
                    <form className="w-full flex flex-wrap gap-6 items-end mb-6 mt-16">
                        ...
                        <div className="flex flex-col flex-1 w-60">
                            ...
                            <input
                                id="keyword"
                                type="text"
                                placeholder="Search by propertyFields name..."
                                value={keyword}
                                ...
                                {currentRows.length > 0 ? (
                                    ...
                                    <tr>
                                        <td colSpan={4} className="text-center py-4 text-gray-500">
                                            No propertyFields match your search.
                                        </td>
                                    </tr>
                                )}
                            </tbody>
