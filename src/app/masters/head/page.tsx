...
import { headDialogDataInterface, headGetDataInterface } from "@/store/masters/head/head.interface";
import { deleteHead, getHead } from "@/store/masters/head/head";


export default function HeadPage() {
    const [head, setHead] = useState<headGetDataInterface[]>([]);
    const [keyword, setKeyword] = useState("");
    const [limit, setLimit] = useState("10");
    const [isDeleteDialogOpen, setIsDeleteDialogOpen] = useState(false);
    const [deleteDialogData, setDeleteDialogData] = useState<headDialogDataInterface | null>(null);
    ...
    const fetchHead = async () => {
        ...
        if (data) {
            const formattedData = data.map((r: headGetDataInterface) => ({
                ...r,
                Name: r.Name.charAt(0).toUpperCase() + r.Name.slice(1)
            }));
            setHead(formattedData);
        }
    };
    ...
    // Delete head
    const handleDelete = async (data: headDialogDataInterface | null) => {
        if (!data) return;
        const res = await deleteHead(data.id);
        if (res) {
    ...
    const handleEdit = (id?: string) => {
        router.push(`/masters/head/edit/${id}`);
    };
    ...
    const totalTablePages = Math.ceil(filteredHead.length / rowsPerTablePage);
    const indexOfLastRow = currentTablePage * rowsPerTablePage;
    const indexOfFirstRow = indexOfLastRow - rowsPerTablePage;
    const currentRows = filteredHead.slice(indexOfFirstRow, indexOfLastRow);
    ...
    return (
        <MasterProtectedRoute>
            ...
            <div className="min-h-[calc(100vh-56px)] overflow-auto max-md:py-10">
                ...
                {/* Delete Dialog */}
                <DeleteDialog<headDialogDataInterface>
                ...
                <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-200 relative">
                    <PageHeader title="Dashboard" subtitles={["Head"]} />
                    {/* Add Button */}

                    <AddButton
                        url="/masters/head/add"
                        text="Add"
