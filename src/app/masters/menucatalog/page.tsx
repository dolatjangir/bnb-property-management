...
import { deleteMenuCatalog, getMenuCatalog } from "@/store/masters/menucatalog/menucatalog";
import { menuCatalogDialogDataInterface, menuCatalogGetDataInterface } from "@/store/masters/menucatalog/menucatalog.interface";



export default function MenuCatalogPage() {
    const [menuCatalog, setMenuCatalog] = useState<menuCatalogGetDataInterface[]>([]);
    const [keyword, setKeyword] = useState("");
    const [limit, setLimit] = useState("10");
    const [isDeleteDialogOpen, setIsDeleteDialogOpen] = useState(false);
    const [deleteDialogData, setDeleteDialogData] = useState<menuCatalogDialogDataInterface | null>(null);
    ...
    const fetchMenuCatalog = async () => {
        ...
        if (data) {
            const formattedData = data.map((r: menuCatalogGetDataInterface) => ({
                ...r,
                Name: r.Name.charAt(0).toUpperCase() + r.Name.slice(1)
            }));
            setMenuCatalog(formattedData);
        }
    };
    ...
    // Filtered menuCatalog
    const filteredMenuCatalog = useMemo(() => {
        return menuCatalog
            .filter((r) => keyword === "" || r.Name.toLowerCase().includes(keyword.toLowerCase()))
    }, [menuCatalog, keyword, limit]);

    // Delete menuCatalog
    const handleDelete = async (data: menuCatalogDialogDataInterface | null) => {
        if (!data) return;
        const res = await deleteMenuCatalog(data.id);
        if (res) {
            toast.success("MenuCatalog deleted successfully!");
            setIsDeleteDialogOpen(false);
            setDeleteDialogData(null);
            fetchMenuCatalog();
            return;
        }
        toast.error("Failed to delete menuCatalog.");
    };

    // Edit menuCatalog
    const handleEdit = (id?: string) => {
        router.push(`/masters/menucatalog/edit/${id}`);
    };
    ...
    const totalTablePages = Math.ceil(filteredMenuCatalog.length / rowsPerTablePage);
    const indexOfLastRow = currentTablePage * rowsPerTablePage;
    const indexOfFirstRow = indexOfLastRow - rowsPerTablePage;
    const currentRows = filteredMenuCatalog.slice(indexOfFirstRow, indexOfLastRow);
    ...
    return (
        <MasterProtectedRoute>
            ...
            <div className="min-h-[calc(100vh-56px)] overflow-auto max-md:py-10">
                ...
                {/* Delete Dialog */}
                <DeleteDialog<menuCatalogDialogDataInterface>
                ...
                <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-200 relative">
                    <PageHeader title="Dashboard" subtitles={["Menu Catalog"]} />
                    {/* Add Button */}

                    <AddButton
                        url="/masters/menucatalog/add"
                        text="Add"
                                ...
                                {currentRows.length > 0 ? (
                                    ...
                                    <tr>
                                        <td colSpan={4} className="text-center py-4 text-gray-500">
                                            No menuCatalog match your search.
                                        </td>
                                    </tr>
                                )}
                            </tbody>
