...
import { menuCatalogTypeDialogDataInterface, menuCatalogTypeGetDataInterface } from "@/store/masters/menucatalogtype/menucatalogtype.interface";
import { deleteAllMenuCatalogType, deleteMenuCatalogType, getMenuCatalogType } from "@/store/masters/menucatalogtype/menucatalogtype";

interface DeleteAllDialogDataInterface { }

export default function MenuCatalogTypePage() {
  const [menuCatalogType, setMenuCatalogType] = useState<menuCatalogTypeGetDataInterface[]>([]);
  const [keyword, setKeyword] = useState("");
  const [limit, setLimit] = useState("10");
  const [isDeleteDialogOpen, setIsDeleteDialogOpen] = useState(false);
  const [isDeleteAllDialogOpen, setIsDeleteAllDialogOpen] = useState(false);
  const [deleteDialogData, setDeleteDialogData] =
    useState<menuCatalogTypeDialogDataInterface | null>(null);
    const [deleteAllDialogData, setDeleteAllDialogData] =
        useState<DeleteAllDialogDataInterface | null>(null);
  const [currentTablePage, setCurrentTablePage] = useState(1);
  const [rowsPerTablePage, setRowsPerTablePage] = useState(10);
  const [selectedMenuCatalogType, setSelectedMenuCatalogType] = useState<string[]>([]);
  const router = useRouter();

  const fetchMenuCatalogType = async () => {
    const data = await getMenuCatalogType();
    if (data) {
      const formatted = data.map((t: menuCatalogTypeGetDataInterface) => ({
        ...t,
        Name: t.Name.charAt(0).toUpperCase() + t.Name.slice(1),
      }));
      setMenuCatalogType(formatted);
    }
  };

  useEffect(() => {
    fetchMenuCatalogType();
  }, []);
  ...
  const filteredMenuCatalogType = useMemo(() => {
    return menuCatalogType
      .filter(
        (t) =>
          keyword === "" ||
          t.Name.toLowerCase().includes(keyword.toLowerCase()) ||
          t.MenuCatalog?.Name?.toLowerCase().includes(keyword.toLowerCase())
      )
  }, [menuCatalogType, keyword]);

  /* SELECT ALL HANDLER */
  const handleSelectAll = () => {
    const allIds = currentRows.map((c) => c._id);
    setSelectedMenuCatalogType((prev) =>
  ...
  const handleDelete = async (data: menuCatalogTypeDialogDataInterface | null) => {
    if (!data) return;
    const res = await deleteMenuCatalogType(data.id);
    if (res) {
  ...
  const handleEdit = (id?: string) => {
    router.push(`/masters/menucatalogtype/edit/${id}`);
  };

  const handleDeleteAll = async () => {
    if (filteredMenuCatalogType.length === 0) return;
    const payload = {
      menucatalogIds: [...selectedMenuCatalogType]
    }
    const response = await deleteAllMenuCatalogType(payload);
    if (response) {
      toast.success(`All menuCatalogType deleted`);
      setIsDeleteAllDialogOpen(false);
      setDeleteAllDialogData(null);
      setSelectedMenuCatalogType([]);
  ...
  const totalTablePages = Math.ceil(filteredMenuCatalogType.length / rowsPerTablePage);
  const indexOfLastRow = currentTablePage * rowsPerTablePage;
  const indexOfFirstRow = indexOfLastRow - rowsPerTablePage;
  const currentRows = filteredMenuCatalogType.slice(indexOfFirstRow, indexOfLastRow);
  ...
  return (
    <MasterProtectedRoute>
      ...
      <div className="min-h-[calc(100vh-56px)] overflow-auto max-md:py-10">
        ...
        {/* DELETE POPUP */}
        <DeleteDialog<menuCatalogTypeDialogDataInterface>
          isOpen={isDeleteDialogOpen}
          title="Are you sure you want to delete this catalog type?"
        ...
        <DeleteDialog<DeleteAllDialogDataInterface>
          ...
          title="Are you sure you want to delete selected Catalog Type?"
        ...
        <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-200 relative">
          <PageHeader title="Dashboard" subtitles={["Menu Catalog Type"]} />
          {/* Add Button */}

          <AddButton
            url="/masters/menucatalogtype/add"
            text="Add"
              ...
              <div className="flex gap-10 items-center px-3 py-4 min-w-max text-gray-700">
                <label htmlFor="selectall" className=" relative overflow-hidden py-[2px] group hover:bg-[var(--color-primary-lighter)] hover:text-white text-[var(--color-primary)] bg-[var(--color-primary-lighter)]  rounded-tr-sm rounded-br-sm  border-l-[3px] px-2 border-l-[var(--color-primary)] cursor-pointer">
                  ...
                  <button type="button" className=" relative overflow-hidden py-[2px] group hover:bg-[var(--color-primary-lighter)] hover:text-white text-[var(--color-primary)] bg-[var(--color-primary-lighter)]  rounded-tr-sm rounded-br-sm  border-l-[3px] px-2 border-l-[var(--color-primary)] cursor-pointer" onClick={() => {
                      if (filteredMenuCatalogType.length > 0) {
                        if (selectedMenuCatalogType.length < 1) {
                          const firstPageIds = currentRows.map((c) => c._id);
                          setSelectedMenuCatalogType(firstPageIds);
                        }
              ...
              <thead className="bg-[var(--color-primary)] text-white">
                <tr className="flex justify-between items-center w-full">
                  <th className="flex items-center gap-10 px-8 py-3 border border-[var(--color-secondary-dark)] text-left w-1/2">
                    <p className="w-[30px]">
                      <input
                        ...
                        checked={
                          ...
                          currentRows.every((r) => selectedMenuCatalogType.includes(r._id))
                        }
                    ...
                    <p className="w-[200px]">Menu Catalog</p>
              ...
              <tbody>
                {currentRows.length > 0 ? (
                  ...
                  <tr>
                    ...
                    >
                      No customer menuCatalogType found.
                    </td>
                  </tr>
                )}
              </tbody>
